...
export class JSONCompletion {
  ...
  private doCompleteForSchema(ctx: CompletionContext, rootSchema: JSONSchema7) {
    ...
    const prefix = ctx.state
      .sliceDoc(node.from, ctx.pos)
      .replace(/^(["'])/, "");
    ...
    if (
      node &&
      (() => {
        const resolvedName = resolveTokenName(node.name, this.mode);
        return (
          resolvedName === TOKENS.OBJECT || resolvedName === TOKENS.JSON_TEXT
        );
      })() &&
  ...
  private getAppliedValue(value: unknown): { label: string; apply: string } {
 ...
 * @param schema
 */
function makeSchemaLax<T>(schema: T): T {
...
function getEffectiveObjectWithPropertiesSchema(
  ...
  for (const possibleDirectPropertyName of possibleDirectPropertyNames) {
...
function getReferenceSchema(schema: JSONSchema7, ref: string) {
  ...
  let curReference: Record<string, unknown> | undefined =
  ...
  refPath.forEach((cur) => {
    ...
    if (cur === "#") {
      curReference = schema as unknown as Record<string, unknown>;
      return;
    }
    if (typeof curReference === "object") {
      curReference = curReference[cur] as Record<string, unknown> | undefined;
    }
  });
